...
For each bird, we estimate the latent ecological state for each day between the first capture date and the last observation, which occurred when a bird was found dead or after which an alive bird was censored. 
...
Birds were only observed every few days; therefore, we use an indexing variable A to only include data likelihoods from days with observations.
For each day of observations in A, we classify birds into 4 observation states based on the ecological state:
